...
const AddBooking = ({ setIsOpen }) => {
    ...
    // ✅ STABLE COORDS: These live OUTSIDE Formik — they only update when a
    // suggestion is selected or map is clicked. They never change while typing.
    // Passing these to <Maps> means the map is completely immune to keystrokes.
    ...
    // formikRef lets us call setFieldValue from outside Formik's render children
    ...
    // ── Callbacks for map — stable references so memo comparison works ─────────
    ...
    useEffect(() => {
        ...
        const tenant = rawTenant?.data || {};
        if (tenant?.maps_api) {
            setMapsApi(tenant.maps_api.toLowerCase() === "barikoi" ? "barikoi" : "google");
        }
    }, []);
    ...
    useEffect(() => {
        if (SEARCH_API === "google" || SEARCH_API === "both") {
            loadGoogleScript().then(() => setGoogleService(new window.google.maps.places.AutocompleteService()));
        }
    }, [SEARCH_API]);
    ...
    const searchLocation = async (query, type, index = null) => {
        ...
        // ── Google: already country-locked via componentRestrictions ─────────────
        if ((SEARCH_API === "google" || SEARCH_API === "both") && googleService) {
            googleService.getPlacePredictions(
                {
                    input: query,
                    componentRestrictions: { country: COUNTRY_CODE }, // e.g. "in", "gb", "bd"
                },
                (predictions, status) => {
                    if (status === "OK") {
        ...
        // ── Barikoi: add &country_code= to lock results to tenant country ─────────
        if (SEARCH_API === "barikoi" || SEARCH_API === "both") {
            try {
                // COUNTRY_CODE is e.g. "in", "bd", "gb" — Barikoi expects uppercase
    ...
    // ✅ KEY FIX: selectLocation sets stable coords only after suggestion is picked
    // The map will NOT move while the user is typing — only when this completes
    const selectLocation = async (item, type, setFieldValue, index = null) => {
        ...
        if (latLng) {
            ...
            if (type === "pickup") {
                ...
                setStablePickupCoords({ lat: latLng.lat, lng: latLng.lng }); // ✅ map update only here
                ...
                setStableDestinationCoords({ lat: latLng.lat, lng: latLng.lng }); // ✅ map update only here
            } else {
                ...
                setStableViaCoords(prev => { // ✅ map update only here
                    const updated = [...prev];
    ...
    const invalidateFare = () => {
        ...
        // ✅ Do NOT clear stable coords here — map should keep showing last confirmed location
    };
    ...
    const swapLocations = (index, setFieldValue, values) => {
        ...
        // ✅ Swap stable coords too so map reflects the swap
    ...
    // ✅ THE CRITICAL FIX: Memoize the Maps component so it NEVER re-renders
    // when Formik updates (which happens on every keystroke).
    // Maps only re-renders when stablePickupCoords, stableDestinationCoords,
    // or stableViaCoords actually change — which only happens when a suggestion
    // is selected or the map is clicked.
    const memoizedMap = useMemo(() => (
        ...
        />
    ), [mapsApi, stablePickupCoords, stableDestinationCoords, stableViaCoords, SEARCH_API]);
    // ↑ formikSetFieldValue intentionally omitted — map click updates go through
    //   the callbacks which update stable state first, then call setFieldValue
        ...
        <>
            ...
            >
                {({ values, setFieldValue }) => {
                    // ✅ Capture setFieldValue so the memoized map can use it for click events
