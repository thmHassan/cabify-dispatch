...
const GoogleMapSection = ({ mapRef, mapInstance, markers, driverData, setDriverData, socket, countryCenter, plotsData }) => {
...
const BarikoiMapSection = ({ mapRef, mapInstance, markers, driverData, setDriverData, socket, countryCenter, plotsData }) => {
  const [mapReady, setMapReady] = useState(false);
  const { barikoiKey } = getApiKeys();

  const renderPlots = () => {
    if (!mapInstance.current || !plotsData) return;
    // For Barikoi/MapLibre, we'd ideally use sources/layers, but for simplicity we can use polygons if we have a way.
    // However, simplest is to use geojson source.
...
const Overview = () => {
  ...
  useEffect(() => {
    ...
    const handleOnJobDriver = (rawData) => {
      ...
      if (data?.driverName || data?.driver_name) {
        ...
        const obj = { id: Date.now(), name: name, ...data };
  ...
  const mapProps = { mapRef, mapInstance, markers, driverData, setDriverData, socket, countryCenter, plotsData };
