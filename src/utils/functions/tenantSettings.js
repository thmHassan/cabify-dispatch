...
export const getTenantCountryIso = () => {
    const tenant = getTenantData();
    if (!tenant) return null;
    const iso = tenant.country_of_use || tenant.data?.country_of_use;
    return iso ? String(iso).trim().toUpperCase() : null;
};

export const getTenantDialCode = () => {
    const iso = getTenantCountryIso();
    return iso ? (COUNTRY_CODE_MAP[iso] || "") : "";
};

let cachedDistanceUnit = null;

export const resolveDistanceUnitFromApi = (units) => {
    if (!units) return cachedDistanceUnit || "Km";
    return String(units).toLowerCase() === "miles" ? "Miles" : "Km";
};

export const setCachedDistanceUnit = (units) => {
    if (!units) return;
...
export const getTenantDistanceUnit = () => {
    ...
    const tenant = getTenantData();
    const units = tenant?.units || tenant?.data?.units || "km";
    return resolveDistanceUnitFromApi(units);
};
...
export const formatPhoneDisplay = (countryCode, phoneNo) => {
    ...
    const phone = phoneNo || "";
    const combined = `${dial} ${phone}`.trim();
    return combined || "-";
};
